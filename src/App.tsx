...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const don = ss.reduce((a,s)=>a+s.done,0);
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:"system-ui",fontSize:18,color:"#374151"}}>
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",maxWidth:860,margin:"0 auto",padding:14,minHeight:"100vh",background:"#f1f5f9"}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        {["dashboard",...SUBJECTS.map(s=>s.id)].map(t => {
          ...
          return (
            <button key={t} onClick={()=>setTab(t)} style={{padding:"7px 13px",borderRadius:20,border:"none",cursor:"pointer",fontWeight:active?700:500,fontSize:13,background:active?(sub?sub.color:"#1e3a8a"):"white",color:active?"white":"#374151",boxShadow:active?"0 2px 10px rgba(0,0,0,.22)":"0 1px 3px rgba(0,0,0,.08)",transition:"all .18s"}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
          <div style={{background:"white",borderRadius:16,padding:22,width:"100%",maxWidth:400,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
            ...
            {tf.obtained && tf.max && +tf.max>0 && (
              <div style={{textAlign:"center",padding:"8px 0",fontSize:26,fontWeight:800,color:pctColor(pct(+tf.obtained,+tf.max)),marginBottom:8}}>
