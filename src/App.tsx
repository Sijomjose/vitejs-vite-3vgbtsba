...
const SUBJECTS = [
  ...
  },
  {
    ...
    sections: [
      { name: "Kshitij – Kavya Khand (Poetry)", chapters: [
        "Kabir – Sakhiyan aur Sabad","Mirabai – Pad","Bihari – Dohe",
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(d) setData(d);
        ...
        const local = localStorage.getItem("savio_v3");
  ...
  const cd = id => data[id]||{ status:"not_started", revision:false, tests:[], notes:"" };
  const cycleStatus = id=>{ const c=cd(id); const i=S_CYCLE.indexOf(c.status); persist({...data,[id]:{...c,status:S_CYCLE[(i+1)%4]}}); };
  ...
  const hasPapers=(p)=>p&&(p.qp?.some(x=>x)||p.ma?.some(x=>x)||p.as?.some(x=>x));
  const initPapers=(p)=>({ qp:p?.qp?.length?p.qp:[""], ma:p?.ma?.length?p.ma:[""], as:p?.as?.length?p.as:[""] });
  ...
  const stats = useMemo(()=>{
    const ss = SUBJECTS.map(sub=>{
      ...
      const allTests=chs.flatMap(c=>(cd(c.id).tests||[]).map(t=>({...t,cName:c.name})));
      return {id:sub.id,name:sub.name,icon:sub.icon,color:sub.color,total:chs.length,done,prog,rev,tests:allTests};
    });
    const tot=ss.reduce((a,s)=>a+s.total,0), don=ss.reduce((a,s)=>a+s.done,0);
    return {ss, tot, don, pct:tot?Math.round(don/tot*100):0};
  },[data]);

  if(loading) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:"system-ui",fontSize:18,color:"#374151"}}>📚 Loading Savio's Study Tracker…</div>;
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",maxWidth:860,margin:"0 auto",padding:14,minHeight:"100vh",background:"#f1f5f9"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1e3a8a,#6d28d9)",borderRadius:16,padding:"18px 22px",marginBottom:14,color:"white"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontWeight:800,fontSize:22,letterSpacing:-0.5}}>Savio's Study Tracker</div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        {["dashboard",...SUBJECTS.map(s=>s.id)].map(t=>{
          const sub=SUBJECTS.find(s=>s.id===t); const active=tab===t;
      ...
      {tab==="dashboard"&&<>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:10,marginBottom:14}}>
          {stats.ss.map(s=>(
            <div key={s.id} onClick={()=>setTab(s.id)} style={{background:"white",borderRadius:12,padding:"14px 16px",cursor:"pointer",borderTop:`4px solid ${s.color}`,boxShadow:"0 1px 4px rgba(0,0,0,.07)",transition:"transform .15s,box-shadow .15s"}}
              ...
              <div style={{marginTop:8,background:"#f3f4f6",borderRadius:10,height:5}}>
                <div style={{background:s.color,borderRadius:10,height:5,width:`${Math.round(s.done/s.total*100)}%`,transition:"width .5s"}}/>
              </div>
            </div>
          ))}
        </div>

        <div style={{background:"white",borderRadius:12,padding:"12px 16px",marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,.07)"}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:8,color:"#374151"}}>How to use</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {Object.entries(S_CFG).map(([k,v])=>(
              <div key={k} style={{background:v.bg,border:`1px solid ${v.border}`,borderRadius:8,padding:"4px 10px",fontSize:12,color:v.color,fontWeight:600}}>{v.dot} {v.label}</div>
            ))}
            <div style={{background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:8,padding:"4px 10px",fontSize:12,color:"#dc2626",fontWeight:600}}>🚩 Flagged for Revision</div>
            <div style={{background:"#eff6ff",border:"1px solid #93c5fd",borderRadius:8,padding:"4px 10px",fontSize:12,color:"#2563eb",fontWeight:600}}>📝 Has Notes</div>
          </div>
          <div style={{marginTop:8,fontSize:12,color:"#6b7280"}}>Click the status badge on any chapter to cycle its progress. Use "+ Test" to record scores.</div>
        </div>
        ...
        <div style={{background:"white",borderRadius:12,padding:"14px 16px",boxShadow:"0 1px 4px rgba(0,0,0,.07)"}}>
          ...
          {(()=>{
            const all=SUBJECTS.flatMap(sub=>flattenChapters(sub).flatMap(ch=>(cd(ch.id).tests||[]).map(t=>({...t,cName:ch.name,sIcon:sub.icon,sColor:sub.color,sName:sub.name}))));
      ...
      {SUBJECTS.map(sub=>{
        ...
        if(sub.chapters) sections=[{name:null,chs:allCh}];
          ...
          sections=sub.sections.map(sec=>{
            const chs=allCh.slice(i,i+sec.chapters.length); i+=sec.chapters.length;
            return {name:sec.name,chs};
          });
        }
        return <div key={sub.id}>
          <div style={{background:sub.color,borderRadius:12,padding:"13px 18px",marginBottom:14,color:"white",display:"flex",alignItems:"center",gap:12}}>
          ...
          {sections.map(sec=>(
            <div key={sec.name||"m"} style={{marginBottom:20}}>
              {sec.name&&<div style={{fontWeight:700,color:sub.color,fontSize:13,marginBottom:8,paddingBottom:6,borderBottom:`2px solid ${sub.color}33`,display:"flex",alignItems:"center",gap:6}}>
                <span>📌</span>{sec.name}
              </div>}
              {sec.chs.map(ch=>{
                ...
                return <div key={ch.id} style={{background:sc.bg,border:`1px solid ${sc.border}`,borderRadius:10,padding:"10px 13px",marginBottom:7,transition:"all .2s"}}>
                  ...
                  {tests.length>0&&<div style={{marginTop:8,display:"flex",flexWrap:"wrap",gap:5,alignItems:"center"}}>
                    {tests.map(t=>{const p=pct(t.obtained,t.max); return <div key={t.id} style={{background:"white",borderRadius:7,padding:"3px 9px",fontSize:12,border:"1px solid #e5e7eb",display:"flex",gap:5,alignItems:"center"}}>
                      ...
                      <button onClick={()=>delTest(ch.id,t.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#d1d5db",fontSize:11,padding:"0 0 0 2px",lineHeight:1}}>✕</button>
                    </div>;})}
      ...
      {testModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
        <div style={{background:"white",borderRadius:16,padding:22,width:"100%",maxWidth:400,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:11}}>
            <div>
              <div style={{fontSize:13,fontWeight:600,marginBottom:4,color:"#374151"}}>Marks Obtained</div>
              <input type="number" min="0" placeholder="e.g. 18" value={tf.obtained} onChange={e=>setTf({...tf,obtained:e.target.value})} style={inp()}/>
            </div>
            <div>
              <div style={{fontSize:13,fontWeight:600,marginBottom:4,color:"#374151"}}>Max Marks</div>
              <input type="number" min="1" placeholder="e.g. 20" value={tf.max} onChange={e=>setTf({...tf,max:e.target.value})} style={inp()}/>
            </div>
          </div>
      ...
      {noteModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
        <div style={{background:"white",borderRadius:16,padding:22,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
          ...
          <textarea value={noteModal.note} onChange={e=>setNoteModal({...noteModal,note:e.target.value})} placeholder="Study notes, important formulae, reminders, weak areas…" style={inp({minHeight:110,resize:"vertical",marginBottom:14})}/>
      ...
      {paperModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
        <div style={{background:"white",borderRadius:16,padding:22,width:"100%",maxWidth:440,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
          ...
          <div style={{fontSize:12,color:"#9ca3af",marginBottom:16,background:"#f8fafc",borderRadius:8,padding:"8px 12px"}}>
          ...
          {[
            ...
            <div key={key} style={{marginBottom:18,background:bg,border:`1px solid ${border}`,borderRadius:10,padding:"12px 14px"}}>
              ...
              {(paperModal.papers[key]||[""]).map((link,i)=>(
                <div key={i} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
                  ...
                  {link&&<a href={link} target="_blank" rel="noreferrer"
                    style={{background:"white",border:`1px solid ${border}`,borderRadius:7,padding:"6px 8px",fontSize:12,textDecoration:"none",color,whiteSpace:"nowrap"}}>🔗</a>}
